...
and expression_app = function 
   ...
   |  (VClosure(enva,[]),s) -> Runtime.VInt 0
...
and closure_prod env s = function
   ...
   | (cp,_)::lp -> let r = match s with 
        | VStruct((cs,None)::ls) -> if cp=cs then  (closure_prod env  (VStruct(ls)) lp)   else (env,false) 
        | VStruct((cs,Some ps)::ls) -> if cp=cs then  (closure_prod env  (VStruct(ls)) lp)   else (env,false) 
        | _-> (env,false) in r; 
...
and vdefinition env = function 
  ...
  | MutuallyRecursive ((Binding(a_i,_),expr)::l) -> let x= (Env.declare (a_i) env) in   ((Env.define (a_i) (expression x expr) x) ;vdefinition x (MutuallyRecursive(l)))
  | MutuallyRecursive([])-> env
  | _-> failwith"vdefinition non reconnu"
